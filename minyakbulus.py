...
while(1):
    ...
    for j in range(1, y_span-1):
        for i in range(detection_line+1, frame_width):
            if (cue.item(j,i,1) == 255):
                ...
                if (cue.item(j,i-1,1) != 255) and (cue.item(j,i-1,1) != 0):
                    cue.itemset((j,i,1), cue.item(j,i-1,1));
                #else:
                #    cue.itemset((j,i,1), 255);
    ...
    if cue.item(y_mid,detection_line,1) != 255:
        blob_id= cue.item(y_mid+step,detection_line,1);
    else:
