...
for n in range(num_slic):
    if ( len(moments[n])>BLOB): # remove spurious superpixel with area less than 2 px 
        #cx= int(moments[n][:,0][1]) # first elem
        #cy= int(moments[n][:,1][1])
        #render.itemset((cy,cx,0), 255) 
        #cx= int(moments[n][:,0][-1]) # last element
        #cy= int(moments[n][:,1][-1])
...
for m in range(isi):
    ...
    # 'smear kernel' calculation
    kernel= 9
    if (cue.item( int((cy[dest]+cy[orig])/2) ,int((cx[dest]+cx[orig])/2)) !=0):
        kernel= kernel-1
    if (cue.item( int((cy[dest]+cy[orig])/2+1) ,int((cx[dest]+cx[orig])/2)) !=0):
        kernel= kernel-1
    if (cue.item( int((cy[dest]+cy[orig])/2+1) ,int((cx[dest]+cx[orig])/2+1)) !=0):
        kernel= kernel-1
    if (cue.item( int((cy[dest]+cy[orig])/2) ,int((cx[dest]+cx[orig])/2+1)) !=0):
        kernel= kernel-1
    if (cue.item( int((cy[dest]+cy[orig])/2-1) ,int((cx[dest]+cx[orig])/2+1)) !=0):
        kernel= kernel-1
    if (cue.item( int((cy[dest]+cy[orig])/2-1) ,int((cx[dest]+cx[orig])/2)) !=0):
    ...
    if (scribe.has_edge(orig,dest)==False) and (scribe.has_edge(dest,orig)==False) and \
        (orig!=dest) and (kernel>0): # 1 px smear in the main stroke 
        fill='#00FF00' # green in-stroke, RGB
        scribe.add_edge(orig, dest, color=fill, weight=1e1/distance, code=vane)
        print(f'stroke between {orig} and {dest}, {kernel}')
    elif (kernel==0):
        # jumps over the void
        ...
        print(f'tashkeel between {orig} and {dest}, {kernel} ')
